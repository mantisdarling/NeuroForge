  ...
  return (
    <section className="advanced-section" id="studies">
      ...
      <div className="study-grid digit-study">
        ...
        <div className="digit-surface"><div className="surface-heading"><span>INPUT CANVAS / 14 × 14 PROJECTION</span><span className="teal">LOCAL ONLY</span></div><canvas ref={canvasRef} className="digit-canvas" width="196" height="196" aria-label="Draw a handwritten digit" onPointerDown={(event) => { drawing.current = true; event.currentTarget.setPointerCapture(event.pointerId); paint(event); }} onPointerMove={paint} onPointerUp={() => { drawing.current = false; }} onPointerLeave={() => { drawing.current = false; }} /><div className="digit-actions"><button className="secondary-action" onClick={() => setDigitScores(digitSession.predict(readDrawing()))}>Inspect drawing</button><button className="text-action" onClick={clearCanvas}>Clear field</button></div></div>
